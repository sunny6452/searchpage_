...
//import FVListitem from "./FPListitem";
...
const FVList = ({
  ...
  const removeFV = useCallback(
    (id, removeName) => {
      ...
      const FavoriteUpdate = AllList.map(
        (list) =>
          list.htmComNm === removeName.title
            ? {
                ...list,
                heart: !list.heart,
              }
      ...
      setAllFVPrevList((AllFVPrevList) => localFavoriteList);
      setAllList((AllList) => FavoriteUpdate);
      console.log("FavoriteUpdate", FavoriteUpdate);
      ...
      var testList = AllFVPrevList.map((test2) =>
        test2.id === id
          ? {
              id: test2.id,
              favorite: test2.favorite.filter(
                (fvName) => fvName !== removeName
              ),
...
*/
  ...
  console.log("FVList AllFVPrevList", AllFVPrevList);
  return (
    <>
...
export default React.memo(FVList);
